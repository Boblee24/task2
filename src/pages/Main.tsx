...
const Main: React.FC<MainProps> = ({ handleAddToCart }) => {
    ...
    return (
        <div className='px-[2rem] w-full'>
            ...
            <div className="bg-black rounded-lg flex flex-col items-center ">
                {
                    foodData.sides.map((food: Food) => (
                        <div className="relative">
                            ...
                            <button onClick={() => handleAddToCart(food)} className="bg-[#FFF078] absolute top-[8rem] left-[10.5rem] flex items-center rounded-lg py-1 px-2 w-[110px]">
                                <h3 className='text-[#FF0000]'>Buy Now</h3>
                                <img src={cart} className='fill-[red]' alt="" />
                            </button>
                        </div>
